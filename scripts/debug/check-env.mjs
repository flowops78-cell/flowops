...
if (!url || !key) {
  failures.push('Missing VITE_SUPABASE_URL or VITE_SUPABASE_PUBLISHABLE_KEY (set in .env / .env.local).');
} else {
  if (url.includes('your-project') || url.includes('placeholder')) {
    failures.push('VITE_SUPABASE_URL looks like a placeholder.');
  }
  if (key === 'your-anon-key' || key.length < 20) {
    failures.push('VITE_SUPABASE_PUBLISHABLE_KEY looks invalid or placeholder.');
  }
}
