...
### 4) Entity Alignment (`partners`, `partner_entries`)
The entity subsystem keeps external relationships separate from day-to-day activity operations. `partners` and `partner_entries` support structured alignment for referrals, shared arrangements, and external channel activity without mixing those records into core activity tracking.
